...
def calculate_real_expense(weight, delivery_type, is_same_city=True):
    ...
    fuel_cost = weight * 4
    distance_cost = 20 if is_same_city else 50
...
def book_parcel():
    ...
    if request.method == 'POST':
        ...
        sender_city = sender_city.lower()
        sender_pincode = sender_pincode.strip()
        receiver_pincode = receiver_pincode.strip()
...
# SAME CITY VALIDATION
        if sender_city == "mumbai":
            if not sender_pincode.startswith("40") or not receiver_pincode.startswith("40"):
                flash("Both sender and receiver must be in Mumbai", "danger")
                return redirect(url_for('book_parcel'))

        elif sender_city == "chennai":
            if not sender_pincode.startswith("60") or not receiver_pincode.startswith("60"):
                flash("Both sender and receiver must be in Chennai", "danger")
                return redirect(url_for('book_parcel'))
...
def generate_invoice(tracking_id):
    ...
    qr = qrcode.make(parcel['tracking_id'])
    ...
    [
        Paragraph(
            "<b>COURIOSYS COURIER SERVICE</b><br/>Parcel Delivery Invoice",
            ParagraphStyle(
                "header",
                parent=styles["Normal"],
                fontSize=14,
    ...
    ('VALIGN', (0,0), (-1,-1), 'TOP'),
    ('ALIGN', (1,0), (1,0), 'RIGHT'),  # QR to right
    ...
    elements.append(Spacer(1, 20))
    # elements.append(Paragraph("Scan to Track Parcel", center_style))
