...
def calculate_cost(weight, parcel_type, delivery_type):
    ...
    base = 50  # booking charge

    weight_cost = float(weight) * 12   # ₹12 per kg
    ...
    cost = base + weight_cost
    cost += type_charge.get(parcel_type, 20)
    cost += delivery_charge.get(delivery_type, 30)

    return round(cost, 2)

# def calculate_real_expense(weight, delivery_type):
#     base_expense = 30          # packaging + hub handling
#     weight_expense = float(weight) * 6   # fuel + handling per kg
...
#     expense = base_expense + weight_expense
#     expense *= delivery_multiplier.get(delivery_type.lower(), 1.0)

#     return round(expense, 2)
def calculate_real_expense(weight, delivery_type):

    fuel = float(weight) * 6        # fuel cost
    handling = 25                  # packaging + sorting
    staff = 20                     # staff cost
    ...
    expense = fuel + handling + staff
    expense *= delivery_multiplier.get(delivery_type, 1.0)
...
def calculate_real_expense(weight, delivery_type):
    ...
    expense = (base + weight_cost) * multiplier.get(delivery_type, 1.0)
    return round(expense, 2)
# def calculate_expense(weight, delivery_type):

#     weight = float(weight)

#     base = 40                      # fixed base cost
#     weight_cost = weight * 10      # ₹10 per kg
...
#     other = 15                     # misc cost (fuel, packaging)

#     expense = base + weight_cost + delivery + other
...
#     fuel = weight * 6        # fuel cost per kg
#     handling = 25            # packaging + sorting
#     staff = 20               # staff cost
...
#         # )
#         print("DEBUG RESET EMAIL:", email)
#         print("DEBUG NEW PASSWORD (plain):", new_password)
...
def admin_dashboard():
    ...
    for p in mongo.db.parcels.find({
        ...
        paid_date = p.get("payment_collected_at") or p.get("delivered_at")
        ...
        if paid_date and paid_date >= last_30_days:
            month_revenue += cost
            month_profit += profit

            day = paid_date.strftime("%d %b")
            trend_data[day] += 1
            profit_trend[day] += profit
    ...
    for i in range(30):
        d = (today - timedelta(days=29 - i)).strftime("%d %b")
    ...
    for i in range(30):
        d = (today - timedelta(days=29 - i)).strftime("%d %b")
        profit_labels.append(d)
        profit_values.append(round(profit_trend.get(d, 0), 2))
    ...
    # 📊 PROFIT VS LOSS
    # ======================
    total_expense = month_revenue - month_profit
